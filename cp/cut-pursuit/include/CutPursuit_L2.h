...
class CutPursuit_L2 : public CutPursuit<T>
{
    ...
    void store_binary_labels(std::string step_name, std::vector<bool>& binary_label){
        if(!this->store_bin_labels)
            return;
        std::ofstream o;
        boost::posix_time::ptime timeLocal = boost::posix_time::second_clock::local_time();
        std::string min = std::to_string(timeLocal.time_of_day().minutes());
        std::string sec = std::to_string(timeLocal.time_of_day().seconds());
        std::string fname = std::to_string(this->m_ite) + std::string("_") + step_name + std::string("_") + min + std::string("_") + sec + std::string("_features.csv");
        o.open(fname);
        o << binary_label.at(0);
        for(int i=1; i<binary_label.size(); i++){
            o << "," << binary_label.at(i); 
        }
        o.close();
    }
    ...
    { // split the graph by trying to find the best binary partition
      ...
      // the affectation as well as h_1 and h_2 are computed alternatively
        ...
        uint32_t saturation;
        //stores wether each vertex is B or not (result of binary cut of each component)
        std::vector<bool> binary_label(this->nVertex);
        //initialize the binary partition with kmeans
        this->init_labels(binary_label);

        store_binary_labels(std::string("km"), binary_label);
        ...
        for (uint32_t i_step = 1; i_step <= this->parameter.flow_steps; i_step++)
        {
            ...
            for (uint32_t ind_com = 0; ind_com < nb_comp; ind_com++)
            {
                ...
                // update the binary partitions according to the max flow solution
                for (uint32_t i_ver = 0;  i_ver < this->components[ind_com].size(); i_ver++)
                {
                    // the color indicates the membership of a vertex to the source or sink vertex 
        ...
        store_binary_labels(std::string("mc"), binary_label);
    ...
    { //-----initialize the labelling for each components with kmeans------------------------------
        ...
        #endif
        #ifdef _WIN32
        for (int ind_com = 0; ind_com < nb_comp; ind_com++)
        #else
        for (uint32_t ind_com = 0; ind_com < nb_comp; ind_com++)
        #endif
        {
            ...
            // we calculate  two clusters - points in the component with potential label = true are closer to cluster a
            // the points with potential label = false are closer to the cluster b
            ...
            for (uint32_t init_kmeans = 0; init_kmeans < this->parameter.kmeans_resampling; init_kmeans++)
            {
                ...
                #endif
                {
    				...
    				tmp_energy += energy_array[i_ver];
                }

                // we now generate a random number to determinate which node will be the second kernel (sampled point)
    			T random_sample = ((T)(rand())) / ((T)(RAND_MAX));
                current_energy = tmp_energy * random_sample;
                for (uint32_t i_ver = 0;  i_ver < comp_size; i_ver++)
                {
                ...
                for (uint32_t ite_kmeans = 0; ite_kmeans < this->parameter.kmeans_ite; ite_kmeans++)
                {
                    ...
                    #endif
                    {
                        ...
                        for(uint32_t i_dim=0; i_dim < this->dim; i_dim++)
                        {
                            // compute the distance (in feature space) to the two sampled kernels (points)
                            // distance two the first kernel
                            distance_kernels[0] += pow(vertex_attribute_map(this->components[ind_com][i_ver]).observation[i_dim]
                                                  - kernels[0][i_dim],2);
                            // distance two the second kernel
                            distance_kernels[1] += pow(vertex_attribute_map(this->components[ind_com][i_ver]).observation[i_dim]
                                                  - kernels[1][i_dim],2);
                        }
                        // is this vertex closer two cluster a (first kernel/sampled point)?
                        potential_label[i_ver] = distance_kernels[0] > distance_kernels[1];
                    }
                    // we now have two clusters, lets compute the new cluster centers (kernels) for the next iterations
                ...
                if (current_energy < best_energy)
                {
                    ...
                    //printf("set best_energy to %f in iteration %i\n", best_energy, this->m_ite);
                    for (uint32_t i_ver = 0;  i_ver < comp_size; i_ver++)
                    {
                        // store which vertices are in the first (potential label = true) or the second cluster (potential label = false)
                             ...
                             , const std::vector<bool> & binary_label)
    {
        ...
        for (uint32_t i_ver = 0;  i_ver < this->components[ind_com].size(); i_ver++)
        {
            ...
            // compute the centers of each component of the binary partition in feature space
        ...
        if ((total_weight[0] == 0)||(total_weight[1] == 0))
        {
            // all points of the components are in one cluster of the binary partition
    ...
    {
        // computes the value and weight of a component
        VertexAttributeMap<T> vertex_attribute_map
                                    = boost::get(boost::vertex_bundle, this->main_graph);
        T total_weight = 0;
        ...
        #endif
        #ifdef _WIN32
        for (int ind_ver = 0; ind_ver < this->components[ind_com].size(); ++ind_ver)
        #else
                                             ...
                                             , const VertexDescriptor<T> & comp2) override
    {
        ...
        for(uint32_t i_dim=0; i_dim<this->dim; i_dim++)
        {
            // the weight of a component is its size
            // the value in a dimension is its mean point feature values per dimension
            merge_value[i_dim] =
