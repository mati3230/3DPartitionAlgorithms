...
class CutPursuit
{
    ...
    uint32_t lastStatsIdx;
    uint32_t m_ite;
    bool store_bin_labels; // enable/disable storage of binary labels
    public:      
    ...
    std::pair<std::vector<T>, std::vector<T>> run()
    {
        ...
        // number of ite_main + iterations + exit code 
        uint32_t n_stats = 3;
        // saturation values
        n_stats += (uint32_t) this->parameter.max_ite_main;
        // energy terms
        n_stats += (uint32_t) (2*this->parameter.max_ite_main);
        // component size after split step
        n_stats += (uint32_t) (this->parameter.max_ite_main);
        // component size after merge step
        n_stats += (uint32_t) (this->parameter.max_ite_main);
        if (this->parameter.verbose > 0)
        {
            printf("Store %i stats\n", n_stats);
        }
        this->stats = std::vector<float>(n_stats);

        stats[0] = (float) this->parameter.max_ite_main;
        uint32_t n_ite = 0;
        ...
        {
            this->m_ite = ite_main;
            uint32_t sIdx = 3 + n_ite;
            uint32_t e1Idx = sIdx + this->parameter.max_ite_main;
            uint32_t e2Idx = e1Idx + this->parameter.max_ite_main;
            this->lastStatsIdx = e2Idx + this->parameter.max_ite_main;
            ...
            {
                printf("Energy first: %.2f, second: %.2f, sum: %.2f, old: %.2f\n", energy.first, energy.second, energy.first + energy.second, old_energy);
            ...
            stats[sIdx] = (float) (100*saturation / (float) this->nVertex);
            stats[e1Idx] = (float) energy.first;
            stats[e2Idx] = (float) energy.second;
            stats[1] = (float)(n_ite + 1);
...
//=============================================================================================
    ...
    void initialize()
    {
        ...
        //printf("Added edges for each vertex to source and sink - n edges: %i\n", eIndex);
    ...
    {   //compute the reduced graph, and if need be performed a backward check
        ...
        this->stats[this->lastStatsIdx] = this->components.size();
        ...
        {   //compute the structure of the reduced graph        
            ...
            this->merge(false);
            this->stats[this->lastStatsIdx + this->parameter.max_ite_main] = this->components.size();
        }
        else
    ...
    {   //compute the adjacency structure between components as well as weight and value of each component
        ...
        #endif
        #ifdef _WIN32
        for (int ind_com = 0;  ind_com < this->components.size(); ind_com++)
        #else
        for (uint32_t ind_com = 0;  ind_com < this->components.size(); ind_com++)
        #endif
        {
            // weight is the total weight of the vertices (which should just be the number of vertices in the component)
            ...
            {
                // average feature value in each dimension of a component
    ...
    uint32_t merge(bool is_cutoff)
    {
        ...
        //we store in merge_queue the potential mergeing with a priority on the potential gain (largest first)
        ...
        {   //loop through the potential mergeing and accept the ones that decrease the energy
            ...
            //printf("%f, ", mergeing_information.merge_gain);
            n_merged++;
