...
PyObject * compute_geof(const bpn::ndarray & xyz ,const bpn::ndarray & target, int k_nn, bool verbose=true)
...
PyObject * compute_all_geof(const bpn::ndarray & xyz ,const bpn::ndarray & target, int k_nn, bool verbose=true)
{//compute the following geometric features (geof) features of a point cloud:
 //linearity planarity scattering verticality
    std::size_t n_ver = bp::len(xyz);
    std::vector< std::vector< float > > geof(n_ver, std::vector< float >(11,0));
    //--- read numpy array data---
    const uint32_t * target_data = reinterpret_cast<uint32_t*>(target.get_data());
    const float * xyz_data = reinterpret_cast<float*>(xyz.get_data());
    std::size_t s_ver = 0;
    ...
    {//each point can be treated in parallell independently
        ...
        //--- compute the verticality feature---
        geof[i_ver][8] = (float)v3[0];
        geof[i_ver][9] = (float)v3[1];
        geof[i_ver][10] = (float)v3[2];
        std::vector<float> unary_vector =
            {lambda[0] * fabsf(v1[0]) + lambda[1] * fabsf(v2[0]) + lambda[2] * fabsf(v3[0])
            ,lambda[0] * fabsf(v1[1]) + lambda[1] * fabsf(v2[1]) + lambda[2] * fabsf(v3[1])
            ,lambda[0] * fabsf(v1[2]) + lambda[1] * fabsf(v2[2]) + lambda[2] * fabsf(v3[2])};
        ...
        float verticality = unary_vector[2] / norm;
        //float verticality0 = unary_vector[0] / norm;
        //float verticality1 = unary_vector[1] / norm;
        float eigensum = lambda[0] + lambda[1] + lambda[2];
        float omnivariance = lambda[2] / eigensum;
        //float omnivariance = lambda[0] / eigensum;
        //float middle_var = lambda[1] / eigensum;
        //float curvature = lambda[2] / eigensum;
        float anisotropy = (lambda[0] - lambda[2]) / lambda[0];
        //float anisotropy1 = (lambda[1] - lambda[2]) / lambda[0];
        float eigenentropy = -(lambda[0]*std::log(lambda[0]) + lambda[1]*std::log(lambda[1]) + lambda[2]*std::log(lambda[2]));
        //---fill the geof vector---
        geof[i_ver][0] = linearity;
        geof[i_ver][1] = planarity;
        geof[i_ver][2] = scattering;
        ...
        geof[i_ver][5] = eigensum;
        geof[i_ver][6] = anisotropy;
        geof[i_ver][7] = eigenentropy;
        //geof[i_ver][8] = curvature;
        //geof[i_ver][9] = middle_var;
        //geof[i_ver][10] = verticality0;
        //geof[i_ver][11] = verticality1;
        //geof[i_ver][12] = anisotropy1;
        //---progression---
        s_ver++;//if run in parellel s_ver behavior is udnefined, but gives a good indication of progress
        if (verbose){
            if (s_ver % 10000 == 0)
...
PyObject * compute_curv_normals(const bpn::ndarray & xyz ,const bpn::ndarray & target, int k_nn, bool verbose=true)
{//compute the following geometric features (geof) features of a point cloud:
 //linearity planarity scattering verticality
    std::size_t n_ver = bp::len(xyz);
    ...
    {//each point can be treated in parallell independently
        ...
        float scattering =  sqrtf(lambda[2]) / sqrtf(lambda[0]);
        //--- compute the verticality feature---
        geof[i_ver][5] = (float)v3[0];
        geof[i_ver][6] = (float)v3[1];
        geof[i_ver][7] = (float)v3[2];
        std::vector<float> unary_vector =
            {lambda[0] * fabsf(v1[0]) + lambda[1] * fabsf(v2[0]) + lambda[2] * fabsf(v3[0])
            ,lambda[0] * fabsf(v1[1]) + lambda[1] * fabsf(v2[1]) + lambda[2] * fabsf(v3[1])
            ,lambda[0] * fabsf(v1[2]) + lambda[1] * fabsf(v2[2]) + lambda[2] * fabsf(v3[2])};
        float norm = sqrt(unary_vector[0] * unary_vector[0] + unary_vector[1] * unary_vector[1]
                        + unary_vector[2] * unary_vector[2]);
        float verticality = unary_vector[2] / norm;
        float curvature = lambda[2] / (lambda[0] + lambda[1] + lambda[2]);
        //---fill the geof vector---
        geof[i_ver][0] = linearity;
        geof[i_ver][1] = planarity;
        geof[i_ver][2] = scattering;
        geof[i_ver][3] = verticality;
        geof[i_ver][4] = curvature;
...
PyObject * compute_geof_var(const bpn::ndarray & xyz ,const bpn::ndarray & target, const bpn::ndarray & n_neighbours, bool verbose=true)
{//compute the following geometric features (geof) features of a point cloud:
 //linearity planarity scattering verticality
    std::size_t n_ver = bp::len(xyz);
    std::vector< std::vector< float > > geof(n_ver, std::vector< float >(4,0));
    ...
    const uint32_t * target_data = reinterpret_cast<uint32_t*>(target.get_data());
    const float * xyz_data = reinterpret_cast<float*>(xyz.get_data());
    const uint32_t * neigh_data = reinterpret_cast<uint32_t*>(n_neighbours.get_data());
    std::size_t s_ver = 0;
    std::size_t i_edg = 0;
    #pragma omp parallel for schedule(static)
    for (std::size_t i_ver = 0; i_ver < n_ver; i_ver++)
    {//each point can be treated in parallell independently
        //--- compute 3d covariance matrix of neighborhood ---
        std::size_t ind_nei;
        uint32_t n_nei = neigh_data[i_ver];
        ei::MatrixXf position(n_nei+1,3);
        position(0,0) = xyz_data[3 * i_ver];
        position(0,1) = xyz_data[3 * i_ver + 1];
        position(0,2) = xyz_data[3 * i_ver + 2];

        for (std::size_t i_nei = 0; i_nei < n_nei; i_nei++)
        {
                //add the neighbors to the position matrix
            ind_nei = target_data[i_edg];
            position(i_nei+1,0) = xyz_data[3 * ind_nei];
        ...
        // compute the covariance matrix
        ei::MatrixXf centered_position = position.rowwise() - position.colwise().mean();
        ei::Matrix3f cov = (centered_position.adjoint() * centered_position) / float(n_nei + 1);
        ei::EigenSolver<Matrix3f> es(cov);
        //--- compute the eigen values and vectors---
        std::vector<float> ev = {es.eigenvalues()[0].real(),es.eigenvalues()[1].real(),es.eigenvalues()[2].real()};
        std::vector<int> indices(3);
...
BOOST_PYTHON_MODULE(libply_c)
{
    ...
    def("compute_all_geof", compute_all_geof);
    def("compute_geof_var", compute_geof_var);
    def("compute_curv_normals", compute_curv_normals);
    def("prune", prune);
