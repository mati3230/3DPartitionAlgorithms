...
#include "../vccs/m_supervoxel_clustering.hpp"

#include <pcl/PolygonMesh.h>
#include <pcl/conversions.h>

#include "../vccs/supervoxel_clustering_geo.h"
 ...
 namespace pcl
 {
   ...
   class  SuperVoxelBasedSegmentation: public pcl::octree::OctreePointCloud<PointT>
   {
	...
	public:
		...
		//Construction
		SuperVoxelBasedSegmentation(double input_resolution)
		...
		void
		setGraphSize(double large_resolution)
		{
		...
		createSupervoxels()
		{
			...
			std::vector<std::vector<int> > supervoxels_points_idx;

			//Creat supervoxel structure
			//源自 Papon CVPR 2013 的实现, 好像不能调, 只能后续分析结果...
			pcl::MSupervoxelClustering<pcl::PointXYZRGBA> super (this->voxel_resolution_, this->seed_resolution_);

			//printf("SVGS: Super Input\n");
			...
			std::map <uint32_t, pcl::MSupervoxel<pcl::PointXYZRGBA>::Ptr > supervoxel_clusters;

			//printf("SVGS: Extraction\n");
			//Oversegmentation using supervoxel
			super.extract (supervoxel_clusters);
			//printf("SVGS: Refinement\n");
			...
			//printf("%i, %i\n", size_labeled_points, (int)input_cloud->points.size());
			std::vector<int> labels_voxels_idxs;

			//printf("SVGS: Label Maps\n");
			//Create label maps
			...
			for(int k=0;k<max_label;k++)
			{
				if(points_label_map[k].size()>0)
				{
					...
					for(int j=0; j<points_label_map[k].size(); j++){
						point2sp.insert(std::pair<int, int> (points_label_map[k][j], temp_sv_num));
					}
			...
			bool precalc)
		{
			...
			//Creat supervoxel structure
			//源自 Papon CVPR 2013 的实现, 好像不能调, 只能后续分析结果...
			//printf("init clustering\n");
			pcl::SupervoxelClusteringGeo<pcl::PointXYZRGBA> super (
				this->voxel_resolution_,
				this->seed_resolution_,
        uni_source,
        uni_index,
        uni_counts,
        source,
		...
		void
		segmentSupervoxelCloudWithGraphModelGeo(
			bpn::ndarray source_nei, bpn::ndarray target_nei, bpn::ndarray uni_source, bpn::ndarray uni_index,
			bpn::ndarray uni_counts, bpn::ndarray distances, PCNPtr normal_cloud,
			float sig_a, float sig_b, float sig_l, float cut_thred,
			float sig_p, float sig_n, float sig_o, float sig_e, float sig_c, float sig_w, float r_search_gain,
			bool precalc)      //Segmentation
		{
			//Parameter setting
			this->color_impt_=sig_a;
			this->spatial_impt_=sig_b;
			...
			//printf("SVGS: Create\n");
			//Create supervoxels
			this->createSupervoxelsGeo(source_nei, target_nei, uni_source, uni_index, uni_counts, distances, normal_cloud, r_search_gain, precalc);
			...
			//Find neighbors
			//this->findAllSupervoxelNeighbors();
			this->findAllSupervoxelNeighbors(points_cloud_, target_nei, uni_index, uni_counts, distances);
			//printf("%i, %i, %i, %i\n", (int)bp::len(source_adj), (int)bp::len(target_adj), (int)bp::len(source_nei), (int)bp::len(target_nei));
			//printf("SVGS: Conn\n");
			//Connectivity calculation
			for(int i=0;i<this->supervoxels_num_;i++)
			{
				...
				if(this->supervoxel_used_[i])//Supervoxel used or not
				{
					...
					//printf("SVGS: Graph building\n");
					//Graph building
					supervoxel_adjgraph=this->buildAdjacencyGraphGeo(supervoxel_adjidx, sig_p, sig_n, sig_o, sig_e, sig_c, sig_w);
					//printf("SVGS: Graph based segmentation\n");
					//Graph based segmentation
					supervoxel_connect=this->cutGraphSegmentation(cut_thred,supervoxel_adjgraph,supervoxel_adjidx);
					//printf("SVGS: Store the connection information\n");
					//Store the connection information
					supervoxels_connect_idx_.push_back(supervoxel_connect);
			...
			//Cross validation
 			this->crossValidation();
			//printf("SVGS: Closest Check\n");
			//Closest checking
			this->closestCheck(sig_p, sig_n, sig_o, sig_e, sig_c, sig_w);
			//Voxel Clustering
			//printf("SVGS: SV Clustering\n");
			this->clusteringSupervoxels();
			//printf("SVGS: Done\n");
		}
    ...
    private:
		...
		std::vector<std::vector<int> > voxels_adjacency_idx_, supervoxels_neighbor_idx_;				//Idx of the adjacent voxels, adjacent supervoxels, neighboring supervoxels
    ...
    std::map<int, std::map<int, float>> spatial_distances_;
		...
		findAllSupervoxelNeighbors()
		{
			...
			for (int i = 0; i<this->supervoxels_num_; i++)
			{
				...
				if ( supervoxel_centroids_kdtree_.radiusSearch (search_point, search_radius, pointIdxRadiusSearch, pointRadiusSquaredDistance) > 0 )
				{
					...
					this->supervoxels_neighbor_idx_.push_back(std::vector<int>(pointIdxRadiusSearch.size()+1,0));
		...
		void
		findAllSupervoxelNeighbors(PCXYZPtr input_cloud, bpn::ndarray target, bpn::ndarray uni_index, bpn::ndarray uni_counts, bpn::ndarray distances)
		{
			pcl::KdTreeFLANN<pcl::PointXYZ> point_kdtree;
			point_kdtree.setInputCloud (input_cloud);	

			supervoxels_neighbor_idx_.clear();
			for(uint32_t i = 0; i < supervoxels_num_; i++){
				std::vector<int> neighbours;
				supervoxels_neighbor_idx_.push_back(neighbours);
			}
      ...
      std::vector<float> distance;
			for(uint32_t i = 0; i < getVoxelNum(); i++){
				...
				for(uint32_t j = start; j < stop; j++){
					...
					if (distance > graph_resolution_)
						continue;
					int vox_idx_v = (int)point2sp[pindex];
					int vox_idx_u = (int)point2sp[tindex];
			...
			for (std::map<int, std::set<int>>::iterator it = vox_adj.begin(); it != vox_adj.end(); ++it){
				...
				supervoxels_neighbor_idx_[vox_idx].push_back((int)neighbours.size());
				for(std::set<int>::iterator setit = neighbours.begin(); setit != neighbours.end(); ++setit)
				{
					supervoxels_neighbor_idx_[vox_idx].push_back(*setit);
				} 
			}
		}
		...
		getOneSupervoxelNeighbor(int supervoxel_id)
		{
			...
			if (supervoxels_neighbor_idx_[supervoxel_id].size() == 0)
				return neighbor_idx;
		...
		Eigen::MatrixXf
		buildAdjacencyGraphGeo(std::vector<int> spvoxels_idx, float sig_p, float sig_n, float sig_o, float sig_e, float sig_c, float sig_w)//Building simple adjacency Graph: static graph only connectivity considered
		{
			//Settings
			Eigen::MatrixXf adj_matrix; //Adjacency in a n*n*n cube
			std::vector<float> spvoxel1_position, spvoxel1_normal, spvoxel1_eigen;
			...
			for (int i=0;i<neighbor_num;i++)
			{
				...
				for(int j=0;j<neighbor_num;j++)
				{
					...
					if(i!=j)
					{
						dist_all=measuringDistanceGeo(spvoxels_idx[j], spvoxels_idx[j], spvoxel1_position, spvoxel2_position, spvoxel1_normal, spvoxel2_normal, spvoxel1_eigen, spvoxel2_eigen);
