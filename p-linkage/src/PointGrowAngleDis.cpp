		...
		*/
		...
		while( count < clusterNew.size() )
		{
			...
			for( int j = 0; j < num; ++j )
			{
				...
				Eigen::Vector3d pt;
				pt << this->pointData.pts[idx].x, this->pointData.pts[idx].y, this->pointData.pts[idx].z;
				h_mean += pt;
			}
			h_mean *= ( 1.0 / num );
			...
			for( int j = 0; j < num; ++j )
			{
				...
				//cv::Matx<double, 1, 1> OD_mat = ( pt - h_mean ).transpose() * pcaInfos[idxSeed].normal;
				Eigen::Matrix<double, 1, 1> OD_mat = ( pt - h_mean ).transpose() * pcaInfos[idxSeed].normal;
			...
			for( int j = 0; j < num; ++j )
			{
				...
				if ( ODs[j] < ODth && EDs[j] < EDth )
				{
					...
					double angle = acos( normalCur(0) * normalStart(0)
						+ normalCur(1) * normalStart(1)
						+ normalCur(2) * normalStart(2) );
					if (angle != angle)
					{
						continue;
					}
